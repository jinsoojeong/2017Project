...
{
	Log(L"* send [%S] packet size [%d]", this->clientAddress().c_str(), transferSize);
}
...
void ASIOSession::onRecv(size_t transferSize, const boost::system::error_code &error)
{
	if (error) {
		if (error == error::eof) {
            Log(L"* send close packet to client [%S]", this->clientAddress().c_str());
		}
		else {
			Log(L"! error no : %d, msg: %S", error.value(), error.message().c_str());
		}
		return;
	}
	...
	str_t recvMessage = receiveBuffer_.data();
    {
        ...
        *len = '\0';
		Log(L"* [%d] recved(%d) : %s", GET_CURRENT_THREAD_ID(), transferSize, wbuf);
    }
