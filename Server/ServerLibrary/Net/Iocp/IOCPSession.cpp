...
bool IoData::setData(Stream &stream)
{
	...
	if (buffer_.max_size() <= stream.size()) {
		Log(L"! packet size too big [%d]byte", stream.size());
		return false;
	}
	...
	// packet obfuscation
	PacketObfuscation::GetSingleton().encodingHeader((Byte*)buf, packetHeaderSize);
...
void IOCPSession::checkErrorIO(DWORD ret)
{
	if (ret == SOCKET_ERROR
        ...
        Log(L"! socket error: %d", WSAGetLastError());
	}
}
...
Package *IOCPSession::onRecv(size_t transferSize)
{
	...
	PacketObfuscation::GetSingleton().decodingData(packetData, packetDataSize);
	Packet *packet = PacketAnalyzer::GetSingleton().analyzer((const char *)packetData, packetDataSize);
	if (packet == nullptr) {
		Log(L"! invaild packet");
		this->onClose(true);
		return nullptr;
	}
