...
LockSafeScope::LockSafeScope(Lock *lock, LPCWSTR fileName, int lineNo)
{
	...
	lock_ = lock;
	Lock *deadLock = LockManager::GetSingleton().checkDeadLock(lock_);
	if (deadLock != nullptr) 
	{

#ifdef STERN_MODE
		ErrorLog(L"! [%s]lock and [%s]lock is dead detecting!", deadLock->name(), lock->name());
#else
		std::lock(lock_->mutex(), deadLock->mutex());
		Log(L"!!! [%s]lock and [%s]lock is dead detecting!", deadLock->name(), lock->name());
#endif
		return;
	}
